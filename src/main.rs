...
impl State for Div {
    ...
    fn init<P: State>(builder: &mut Builder<P>, param: &Self::Param) -> Self {
        Self { color: [255, 0, 0, Self::DA], i: param.0 }
    }
...
impl Subscriber<i32> for Div {
    fn observe(&mut self, event: &i32, queue: &UpdateQueue) {
        if event % self.i == 0 {
...
impl State for App {
    ...
    fn init<P: State>(builder: &mut Builder<P>, _: &()) -> Self {
        ...
        Self {
            states: (1..=128).into_iter().map(|i| builder.frame((i, state_event.clone()), g, None, None)).collect(),
            state_event,
            i: 0,
        }
    }
    ...
    fn observe(&mut self, event: &MouseEvent, queue: &UpdateQueue) {
        match event {
            ...
            MouseEvent::Click(MouseButton::Left(ButtonState::Released)) => {
                self.states.iter_mut().for_each(|c: &mut Component<Div>| c.update(false, queue));  
